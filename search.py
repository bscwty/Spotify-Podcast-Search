    ...
    for hit in query_results["hits"]["hits"]:
        ...
        if ep_id not in existed_content:
            if n == 1:
                ...
                offset = pre_offset
                search_results.append([ep_id, score, show_name, ep_name, words, offset])
                result_num += 1
            else:

                max_chunk_score, text, offset, offset_range = query(client, ep_id, query_string, pre_offset, n)
                existed_content[ep_id] = []
                existed_content[ep_id].extend(offset_range)
                search_results.append([ep_id, max_chunk_score, show_name, ep_name, text, offset])
                result_num += 1
        else:
            if n == 1:
